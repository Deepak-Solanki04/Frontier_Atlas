...
import { Trophy, Cpu, Layers, ExternalLink, Code2, Check, Copy, ArrowLeft, Share2, Sparkles, BookOpen } from "lucide-react";
...
export default function ModelDetailPage({
  ...
  const handleCopyQuickstart = () => {
    if (model?.quickstart) {
      navigator.clipboard.writeText(model.quickstart);
      setCopied(true);
  ...
  const relatedPapers = useMemo(() => {
    ...
    return list.filter((paper: any) => {
      ...
      if (title.includes(modelNameLower) || abstract.includes(modelNameLower)) {
        return true;
      }
      // Match org or key domain tags
      if (title.includes(orgLower) || abstract.includes(orgLower)) {
        return true;
      }
      return tagsLower.some((t) => title.includes(t) || abstract.includes(t));
    }).slice(0, 10); // Show top 10 most relevant papers
  }, [model]);
    ...
    return (
      <div className="min-h-screen bg-[#F8F7F2] flex items-center justify-center p-4">
        <div className="flex flex-col items-center gap-3 animate-pulse text-center">
          ...
          <p className="text-[14px] text-[#8B8B8B] font-mono">Loading model architecture...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8F7F2] text-[#111111] pb-24 selection:bg-[#FF5A1F] selection:text-white">
      ...
      <header className="sticky top-0 z-40 bg-[#F8F7F2]/90 backdrop-blur-md border-b border-[#EAE9E4]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          >
            ...
            <span className="text-[#111111] font-extrabold truncate max-w-[180px] sm:max-w-none">{model.name}</span>
          </Link>

          <div className="flex items-center gap-2 sm:gap-3">
            <button
              onClick={handleShare}
              className="flex items-center gap-1.5 px-3.5 py-2 bg-white hover:bg-[#E5E5E0] border border-[#E5E5E0] rounded-xl text-[12px] font-bold text-[#333333] transition-colors"
            >
              <Share2 size={14} />
              <span>{shareCopied ? "Link Copied!" : "Share"}</span>
            </button>
            <Link
              href={`/${model.area === "Agentic Coding" ? "agents" : "trending"}`}
              className="hidden sm:flex items-center gap-1.5 px-4 py-2 bg-[#FF5A1F] hover:bg-[#E0462D] text-white rounded-xl text-[12px] font-bold transition-colors shadow-sm"
            >
              <span>Explore {model.area}</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          {/* LEFT SIDEBAR NAVIGATION (Hidden on mobile, 3 cols on large screens) */}
          <div className="hidden lg:block lg:col-span-3 sticky top-24">
            <Sidebar />
          </div>

          {/* RIGHT MAIN DEEP DIVE COLUMN (12 cols on mobile, 9 cols on desktop) */}
          <div className="col-span-1 lg:col-span-9 space-y-8 sm:space-y-12">
                    ...
                    <div key={i} className="space-y-2">
                      <div className="flex items-center justify-between text-[14px] sm:text-[15px] font-bold">
                        <span className="text-[#333333] flex items-center gap-2">
                          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: bm.color || "#FF5A1F" }} />
                          {bm.name}
                        </span>
            ...
            {/* 4. INFERENCE QUICKSTART CODE SNIPPET (Mobile Responsive Scroll) */}
            {model.quickstart && (
              <div className="bg-white border border-[#E5E5E0] rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4 pb-4 border-b border-[#E5E5E0]">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-[13px] font-bold px-4 py-2 bg-[#F8F7F2] hover:bg-[#E5E5E0] border border-[#EAE9E4] rounded-xl transition-colors active:scale-95"
                  >
                    {copied ? (
                      <>
