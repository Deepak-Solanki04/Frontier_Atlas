...
export default function ModelVendorPage({
  params,
}: {
  ...
  const vendorName = decodeURIComponent(resolvedParams.vendor);
  ...
  }, [allModels, vendorName]);

  const vendorTopBoxes = useMemo(() => {
    const vLower = vendorName.toLowerCase();
  ...
  const breadcrumbs = ["Home", "Directory", "Models", vendorName];
      ...
      <div className="models-directory-container">
        ...
        <div className="mb-8">
          <span className="text-[11px] font-mono uppercase tracking-wider px-2.5 py-1 bg-[#FFF6F3] text-[#FF5A1F] rounded border border-[#FFEDD5] font-extrabold mb-3 inline-block">
            § 01 &middot; {vendorName} Registry
          </span>
          <h1 className="text-[36px] sm:text-[46px] font-black text-[#111111] tracking-tight leading-tight mb-3">
            {vendorName} Foundation Models
          </h1>
          <p className="text-[16px] text-[#555555] font-medium max-w-3xl leading-relaxed">
            Explore {filteredCatalogModels.length > 0 ? filteredCatalogModels.length : "indexed"} verified benchmark evaluation records, architectural specifications, and reasoning capabilities for {vendorName} models.
          </p>
        </div>

        {/* Top Evaluated Leaderboard for THIS vendor */}
        {vendorTopBoxes.length > 0 && (
          <div className="mb-12">
            <div className="topic-benchmarks-header">
              <h2 className="topic-benchmarks-title">
                Top Evaluated {vendorName} Foundation Models
              </h2>
              <span className="topic-benchmarks-badge">
                {vendorTopBoxes.length}
              </span>
            </div>
            <div className="topic-benchmarks-grid">
              {vendorTopBoxes.map((card, i) => (
                <Link
                  key={i}
                  href={`/models/${card.id}`}
        ...
        <div className="models-catalog-section mt-6 mb-16 bg-white border border-[#E5E5E0] rounded-[18px] p-6 sm:p-10 shadow-sm">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-[11px] font-mono uppercase tracking-wider px-2.5 py-0.5 bg-[#FFF6F3] text-[#FF5A1F] rounded border border-[#FFEDD5] font-extrabold">
                  § 01 &middot; {vendorName} Complete Index
                </span>
              ...
              <h2 className="text-[28px] sm:text-[34px] font-black text-[#111111] tracking-tight leading-tight">
                {filteredCatalogModels.length} models from {vendorName} &middot; <span className="font-medium text-[#555555]">Every model, measured.</span>
              </h2>
            </div>
            <Link
              href="/models"
          ...
          {filteredCatalogModels.length === 0 ? (
            <div className="py-12 text-center bg-[#F8F7F2] rounded-xl border border-dashed border-[#EAE9E4]">
              <p className="text-[15px] font-bold text-[#555555]">No detailed evaluation records currently indexed for {vendorName} in our offline snapshot.</p>
